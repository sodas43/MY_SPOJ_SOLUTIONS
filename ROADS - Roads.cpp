...
// for this problem we also need to take toll. so need to modify above to
// <src, {dst, {weight, toll}}>
...
int dijkstra(int totCoins, int totCities) {
	...
	while (!minHeap.empty()) {
		...
		// len should be 1st item because based on this minHeap is working here.
		// to change the ordering need to create our own cmp function.
		// Otherwise changing to other ones as 1st element is not working
